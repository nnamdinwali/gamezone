...
export default function App() {
  ...
  const createGame = async () => {
    setError("");
    const rewardEntered = gameForm.rewardPerMinute.trim() !== "";
    const reward = rewardEntered ? Number(gameForm.rewardPerMinute) : null;
    if (!gameForm.title.trim() || !gameForm.description.trim() || !gameForm.storeUrl.trim() || (rewardEntered && (!Number.isFinite(reward) || (reward as number) <= 0))) {
      setError("Enter the game name, description, and Store URL. Reward per minute is optional, but if set it must be greater than zero.");
      return;
    }
    if (!coverImage) {
    ...
    try {
      ...
      if (rewardEntered) payload.append("rewardPerMinute", gameForm.rewardPerMinute);
